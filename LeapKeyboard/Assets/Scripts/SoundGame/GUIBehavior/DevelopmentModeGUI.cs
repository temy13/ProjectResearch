...
public class DevelopmentModeGUI : MonoBehaviour {
	...
	public void BeginVisualization()
	{
		...
		m_actionInfoRegionSeeker.SetSequence(m_musicManager.currentSongInfo.onKeyPositionRegionSequence);
	...
	void OnGUI(){
		...
		GUI.Label(new Rect( 5, 440, 150, 40 ),
			"Nearest(beat):"
		          + m_musicManager.currentSongInfo.onKeyPositionSequence[m_scoringManager.m_lastResult.markerIndex].triggerBeatTiming.ToString());
		GUI.Label(
			new Rect( 150, 440, 150, 40 ),
			"Nearest(index):"
			+ m_musicManager.currentSongInfo.onKeyPositionSequence[m_scoringManager.m_lastResult.markerIndex].line_number.ToString());
		
		// 関連するパート名を表示
		if( m_musicManager.currentSongInfo.onKeyPositionRegionSequence.Count>0 ){
			//現在のパートのインデックスを確認
			...
			{	
				previousHitRegionName
					= m_musicManager.currentSongInfo.onKeyPositionRegionSequence[currentReginIndex].name;
			}
			...
			GUI.Label(new Rect(150, 460, 250, 40), "region ...:" + m_musicManager.currentSongInfo.onKeyPositionRegionSequence[currentReginIndex].name);
		}
